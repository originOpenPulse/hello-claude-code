...
实际代码有 27 种（非原文档的 11 种）。完整列表：PreToolUse, PostToolUse, PostToolUseFailure, PermissionDenied, Notification, UserPromptSubmit, SessionStart, SessionEnd, Stop, StopFailure, SubagentStart, SubagentStop, PreCompact, PostCompact, PermissionRequest, Setup, TeammateIdle, TaskCreated, TaskCompleted, Elicitation, ElicitationResult, ConfigChange, InstructionsLoaded, WorktreeCreate, WorktreeRemove, CwdChanged, FileChanged。

### 12. 补充：4 种 Hook 类型

Command（shell 执行）、Prompt（LLM 评估）、Agent（代理验证）、HTTP（远程 webhook）。
...
### 14. 补充：Session vs Persistent Hooks

Persistent：5 来源（user/project/local/policy/plugins）
Session：3 种注册机制
优先级：user(0) > project(1) > local(2) > policy(3) > plugins(999)

### 15. 补充：Stop Hook 详细机制

包含 last_assistant_message，可通过 JSON {continue:false} 终止。asyncRewake=true 可通过 exit code 2 唤醒模型。

### 16. 补充：Timeout 配置

Tool hooks 默认 10 min，SessionEnd 1500ms。
